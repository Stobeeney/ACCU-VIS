...
def compute_gaze_vector(x, y, center_x, center_y, screen_width=640, screen_height=480):
    ...
    if GL_SPHERE_AVAILABLE:
        try:
            cam_dist = gl_sphere.get_camera_distance()
        except Exception:
            pass
    camera_position = np.array([0.0, 0.0, cam_dist])
...
# State for Far Distance Eye Zoom
zoom_mode = False
eye_roi_center = [320, 240]
eye_roi_size = 180

def on_mouse_click(event, x, y, flags, param):
    global eye_roi_center, zoom_mode
    if event == cv2.EVENT_LBUTTONDOWN:
        eye_roi_center = [x, y]
        zoom_mode = True
        print(f"[Far Mode] Focused on eye at ({x}, {y}) - Zoom ON")
    elif event == cv2.EVENT_RBUTTONDOWN:
        zoom_mode = False
        print("[Far Mode] Full frame mode")
...
def process_camera():
    ...
    global capture_frame_counter
    global stuck_ellipses
    global flip_camera_image, zoom_mode, eye_roi_center, eye_roi_size
    ...
    flip_v = flip_camera_image
    flip_h = False

    cv2.namedWindow("ORiginal Frame")
    cv2.setMouseCallback("ORiginal Frame", on_mouse_click)
    ...
    while True:
        ...
        # Eye Zoom for Far User
        if zoom_mode:
            h, w = frame.shape[:2]
            half = eye_roi_size // 2
            x1 = max(0, min(w - eye_roi_size, eye_roi_center[0] - half))
            y1 = max(0, min(h - eye_roi_size, eye_roi_center[1] - half))
            x2 = min(w, x1 + eye_roi_size)
            y2 = min(h, y1 + eye_roi_size)
            eye_crop = frame[y1:y2, x1:x2]
            in_frame = cv2.resize(eye_crop, (640, 480))
            cv2.rectangle(display_frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.putText(display_frame, f"Far Eye Zoom: ON ({eye_roi_size}px) [Z: toggle, Click: move]", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        else:
            in_frame = frame
            h, w = frame.shape[:2]
            half = eye_roi_size // 2
            x1 = max(0, min(w - eye_roi_size, eye_roi_center[0] - half))
            y1 = max(0, min(h - eye_roi_size, eye_roi_center[1] - half))
            cv2.rectangle(display_frame, (x1, y1), (x1 + eye_roi_size, y1 + eye_roi_size), (0, 255, 255), 1)
            cv2.putText(display_frame, "Click on eye to Zoom (Far Mode) | V: Flip | +/-: Eyeball Dist", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
        ...
        if key == ord('q'):
            ...
            flip_v = not flip_v
            print(f"Vertical Flip: {'ON' if flip_v else 'OFF'}")
        elif key in (ord('h'), ord('H')):
            flip_h = not flip_h
            print(f"Horizontal Flip: {'ON' if flip_h else 'OFF'}")
        elif key in (ord('z'), ord('Z')):
            zoom_mode = not zoom_mode
            print(f"Far Eye Zoom: {'ON' if zoom_mode else 'OFF'}")
        elif key in (ord('+'), ord('=')):
            if GL_SPHERE_AVAILABLE:
                cur = gl_sphere.get_camera_distance()
                gl_sphere.set_camera_distance(cur + 0.5)
                print(f"Eyeball distance: {gl_sphere.get_camera_distance():.1f}")
        elif key in (ord('-'), ord('_')):
            if GL_SPHERE_AVAILABLE:
                cur = gl_sphere.get_camera_distance()
                gl_sphere.set_camera_distance(max(1.5, cur - 0.5))
                print(f"Eyeball distance: {gl_sphere.get_camera_distance():.1f}")
        elif key == ord('['):
            eye_roi_size = max(60, eye_roi_size - 20)
            print(f"Eye crop size: {eye_roi_size}px")
        elif key == ord(']'):
            eye_roi_size = min(400, eye_roi_size + 20)
            print(f"Eye crop size: {eye_roi_size}px")
        elif key == ord('e'):
    ...
    if GL_SPHERE_AVAILABLE and gl_sphere.window:
        try:
            gl_sphere.window.close()
        except Exception:
            pass

# Process a selected video file
def process_video():
    global zoom_mode, eye_roi_center, eye_roi_size
    video_path = filedialog.askopenfilename(filetypes=[("Video Files", "*.mp4;*.avi")])

    if not video_path:
        default_test = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "eye_test.mp4"))
        if os.path.exists(default_test):
            video_path = default_test
            print(f"No file chosen. Using default test video: {default_test}")
        else:
    ...
    flip_v = True  # Flipped upside down by default
    flip_h = False
...
def selection_gui():
    global selected_camera, flip_camera_image
    ...
    tk.Checkbutton(root, text="Flip image upside down", variable=flip_var).pack(pady=4)

    def on_start_camera():
        global flip_camera_image
        flip_camera_image = flip_var.get()
        root.destroy()
        process_camera()

    tk.Button(root, text="Start Camera", command=on_start_camera).pack(pady=5)
    tk.Button(root, text="Browse Video (eye_test.mp4)", command=lambda: [root.destroy(), process_video()]).pack(pady=5)
