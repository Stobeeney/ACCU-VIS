...
CV_pupil_x = 0
CV_pupil_y = 0
camera_distance = 5.5  # Eyeball reference distance (further back so it is viewed from further away)

def set_camera_distance(dist):
    global camera_distance
    camera_distance = max(1.5, float(dist))
    if sphere_widget is not None:
...
class SphereWidget(QOpenGLWidget):
    ...
    def paintGL(self):
        ...
        glTranslatef(0.0, 0.0, -camera_distance)  # Move eyeball reference further back
...
def update_sphere_rotation(x, y, center_x, center_y, screen_width=640, screen_height=480):
    ...
    # Camera position is at z = camera_distance
    camera_position = np.array([0.0, 0.0, camera_distance])
    ...
    glFinish()
    try:
        qimg = sphere_widget.grabFramebuffer()
        qimg = qimg.convertToFormat(RGB_FORMAT)
        ptr = qimg.bits()
        ptr.setsize(qimg.sizeInBytes())
        image = np.array(ptr, copy=True).reshape(qimg.height(), qimg.width(), 3)
        return image
